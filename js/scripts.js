...
window.onerror = function (message, source, lineno, colno, error) {
  ...
  // If the game has been running for at least 5 seconds (to avoid initial load errors)
  if (
    document.readyState === "complete" &&
    new Date().getTime() - window.gameStartTime > 5000
  ) {
    resetGameState();
  }
  // Return false to allow the default error handler to run as well
  return false;
};
...
$(document).ready(function () {
  ...
  // Add double-escape key press handler for emergency reset
  var escKeyPressTime = 0;
  $(document).keydown(function (e) {
    if (e.keyCode === 27) {
      // ESC key
      var currentTime = new Date().getTime();
      if (currentTime - escKeyPressTime < 500) {
        // Double press within 500ms
        resetGameState();
        alert("Game state has been reset. You may continue playing.");
      }
      escKeyPressTime = currentTime;
    }
  });
});
...
$(document).on(
  touchEvent,
  "#mobileMapNavContainer .mobileTilePlacementOptions.activeTileOptions .mobileTilePlacementCancel:not(.natureCubeMode)",
  function () {
    cancelTilePlacement("normalTile");
  }
);
...
$(document).on(
  touchEvent,
  "#mobileMapNavContainer .mobileTilePlacementOptions.activeTileOptions .mobileTilePlacementCancel.natureCubeMode",
  function () {
    cancelTilePlacement("natureCubeTile");
  }
);
...
$(document).on(
  touchEvent,
  "#mobileMapNavContainer .mobileTilePlacementOptions.activeTileOptions .mobileTilePlacementConfirm:not(.natureCubeMode)",
  function () {
    confirmTilePlacement();
    activateTokenPlacement("normalToken");
  }
...
$(document).on(
  touchEvent,
  "#mobileMapNavContainer .mobileTilePlacementOptions.activeTileOptions .mobileTilePlacementConfirm.natureCubeMode",
  function () {
    confirmTilePlacement();
    setupNatureCubeTokenSelection();
  }
...
$(document).on(
  touchEvent,
  ".tokenTileContainer.potentialNatureCube.natureCubeToken .tokenContainer:not(.chosenNatureCubeToken)",
  function () {
...
$(document).on(
  touchEvent,
  ".tokenTileContainer.natureCubeClearTokens .tokenContainer:not(.currentTokenAnimation):not(.lockAllTokens)",
  function () {
...
$(document).on(
  touchEvent,
  "#mobileMapNavContainer .mobileTilePlacementOptions.activeTileOptions .mobileTilePlacementRotateClockwise",
  rotateTileClockwiseFunction
);
...
function clearAnyTokensWithNatureCube() {
  ...
  $("#tileTokenContainer").append(
    '<button id="confirmClearSelectedTokens" class="button is-success" disabled="disabled">Clear Tokens</button><button id="selectAllTokensToClear" class="button is-info">Select All</button>'
  );
...
function temporarilyLockMap(timePeriod) {
  lockMap = true;
  clearTimeout(window.globalLockTimeout);
  setTimeout(function () {
    lockMap = false;
  }, timePeriod);
  window.globalLockTimeout = setTimeout(function () {
    lockMap = false;
  }, timePeriod + 2000);
}
...
function cancelTilePlacement(mode) {
  ...
  if (mode == "normalTile") {
    ...
    $(".mapTileContainer .tileContainer.lockedIn").parentToAnimate(
...
function confirmInvalidNatureCubeTokenPlacementFunction() {
  ...
  // fade in the inactive version of the removed wildlife token and the cross to show to the user its about to be removed
  $(
    ".tokenTileContainer.potentialNatureCube.natureCubeToken.chosenNatureCubeTokenParent .tokenContainer.chosenNatureCubeToken .duplicateToken"
  ).fadeIn();
  $(
    ".tokenTileContainer.potentialNatureCube.natureCubeToken.chosenNatureCubeTokenParent .tokenContainer.chosenNatureCubeToken .redCross"
  ).fadeIn();
...
function undoTilePlacementFunction() {
  ...
  if ($(".tokenTileContainer.chosenTokenTileContainer").length) {
    ...
    $(".mapTileContainer.lastPlacedTile .tileContainer").parentToAnimate(
      $(".tokenTileContainer.chosenTokenTileContainer"),
      1000
    );
...
function removeSoloTilesTokens() {
  try {
    ...
    console.error("Error in removeSoloTilesTokens function: ", e);
    resetGameState;
  }
}
...
function setupInitialTokensAndTiles() {
  ...
  for (let k = 0; k < 4; k++) {
    ...
    initialTokenTileHTML +=
      '<div class="tokenContainer" wildlifetoken="' + initialTokens[k] + '">';
    initialTokenTileHTML +=
      '<img class="token" src="img/tokens/' + initialTokens[k] + '.png" />';
...
function endOfGameSetup() {
  ...
  setTimeout(function () {
    ...
    $("#scoringTable-FinalScoringModalTrigger")
      .removeClass("activeScoringItemTrigger")
...
$(document).on(
  ...
  function () {
    ...
    setTimeout(function () {
      $("#" + thisID[0] + "-finalScoringContainer")
        .removeClass("inactiveScoringItem")
        .addClass("activeScoringItem");
...
$(document).on(
  "mouseover",
  ".mobileFinalScoringItemTrigger:not(.lockTrigger).mobileActiveScoringItemTrigger .mobileGoalScoringTransparentLayer",
  function () {
...
function checkForBlanks() {
  ...
  $(
    ".largeWildlifeFinalScoringInfo.finalScoringItem .individualWildlifeScoringInputContainer .individualScoringInput .individualPointsNum"
  ).each(function () {
  ...
  for (const currentWildlife of allWildlife) {
    ...
    $(
      "#tileTokenContainer.finalScoring .finalScoringItem #" +
        currentWildlife +
        "-individualWildlifeScoringInput .individualPointsNum"
    ).html(tokenScoring[currentWildlife].totalScore);
    if (tokenScoring[currentWildlife].totalScore == 1) {
      $(
        "#tileTokenContainer.finalScoring .finalScoringItem #" +
          currentWildlife +
          "-individualWildlifeScoringInput .pluralPoints"
      ).hide();
    } else {
      $(
        "#tileTokenContainer.finalScoring .finalScoringItem #" +
    ...
    for (let i = 0; i < linkedTileSides.length; i++) {
      ...
      if (allPlacedTiles.hasOwnProperty(newTileID)) {
        ...
        if (
          ...
          allPlacedTiles[newTileID].habitatSides[matchedIndexes[1]]
        ) {
...
function generateAllElkLines() {
  ...
  for (let i = potentialElkLines.length - 1; i >= 0; i--) {
    ...
    for (let j = 0; j < potentialElkLines.length; j++) {
      ...
      if (
        checkedIndexCombos.indexOf(thisIndexCombo) == -1 &&
        checkedIndexCombos.indexOf(altIndexCombo) == -1 &&
        i != j
      ) {
...
jQuery.fn.extend({
  ...
  parentToAnimate: function (newParent, duration) {
    ...
    if ($element[0].className == "earnedNatureToken") {
      ...
      $element[0].className == "tileContainer" ||
      $element[0].className == "tileContainer lockedIn" ||
      $element[0].className == "tileContainer chosenNatureCubeTile lockedIn" ||
      $element[0].className == "activeToken"
    ) {
      ...
      $element[0].className == "tileContainer movingElementOpacity" ||
      $element[0].className == "token movingElementOpacity"
    ) {
      temp.css({
        position: "absolute",
        left: oldOffset.left,
...
function resetGameState() {
  ...
  // Hide options panels that might be stuck
  $("#placedTileOptions").removeClass("showOptions");
  $(".mobileTilePlacementOptions.activeTileOptions")
    .addClass("inactiveTileOptions")
    .removeClass("activeTileOptions");

  // Reset token placement states
  $(".wildlifeTokenPotential").removeClass("wildlifeTokenPotential");
  $(".mapTileContainer").removeAttr("wildlifeTokenPotentialType");
